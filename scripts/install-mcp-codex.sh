#!/usr/bin/env bash
# install-mcp-codex.sh — Install a registry MCP tool into Codex MCP config
# Usage: ./scripts/install-mcp-codex.sh <tool-name> [--apply]
# Default mode is dry-run (prints the codex command).
...
if capability.get("install_vector") == "plugin":
    ...
    print("Install/enable the plugin in Codex; standalone MCP install is not used for this entry.")
    sys.exit(0)
...
if transport == "stdio":
    ...
    cmd = ["codex", "mcp", "add", install_name]
    for key, val in sorted(env_map.items()):
        cmd.extend(["--env", f"{key}={val}"])
    cmd.extend(["--", command, *launch_args])
elif transport in {"http", "streamable-http"}:
    url = expand(launcher.get("url", ""))
    if not url:
        print(f"ERROR: missing launcher.url for http transport in {cap_path}")
        sys.exit(1)
    cmd = ["codex", "mcp", "add", install_name, "--url", url]
    bearer_env = launcher.get("bearer_token_env")
    if bearer_env:
        cmd.extend(["--bearer-token-env-var", bearer_env])
else:
    print(f"ERROR: unsupported transport for Codex: {transport}")
    print("Supported: stdio, http/streamable-http")
    sys.exit(1)
...
if args.apply:
    ...
    print("Installed in Codex MCP config.")
else:
